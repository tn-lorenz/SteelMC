...
impl Player {
    ...
    pub(super) fn passenger_ids_for_packet(entity: &dyn Entity) -> Vec<i32> {
        entity
            .passengers()
            .iter()
    ...
    pub(super) fn send_mob_effect_sync_packet(&self, packet: MobEffectSyncPacket) {
        match packet {
    ...
    pub(super) fn send_active_effects_for_vehicle(&self, vehicle: &dyn Entity) {
    ...
    pub(super) fn remove_active_effects_for_vehicle(&self, vehicle: &dyn Entity) {
        let Some(living_vehicle) = vehicle.as_living_entity() else {
            return;
        };
        for effect in living_vehicle.active_mob_effects() {
...
pub(super) fn nullable_game_mode_id(game_mode: Option<GameType>) -> i8 {
